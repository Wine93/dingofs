...
def parse_dingofs_url(url: str) -> DingoFSEndpoint:
    ...
    we slice authority/path by hand. Query parameters become extra dingofs
    gflag overrides and go through parse_qsl for proper percent-decoding.
    """
    ...
    extra: Dict[str, str] = {}
    if query:
        # strict_parsing=True so '?broken' (no '=') raises instead of silently
        # being dropped — flag typos shouldn't be invisible.
        try:
            pairs = parse_qsl(query, keep_blank_values=True, strict_parsing=True)
        except ValueError as e:
            raise ValueError(f"malformed query in {url}: {e}") from e
        extra = dict(pairs)
