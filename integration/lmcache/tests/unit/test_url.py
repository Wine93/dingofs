...
def test_query_params_become_extra_flags():
    ep = parse_dingofs_url(
        "dingofs://mds:6700/grp?cache_rpc_timeout_ms=5000&cache_rpc_max_retry_times=3"
    )
    assert ep.extra_flags == {
...
def test_rejects_malformed_query():
    with pytest.raises(ValueError):
        parse_dingofs_url("dingofs://mds:6700/grp?broken")
