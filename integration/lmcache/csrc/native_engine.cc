...
uint64_t NativeEngine::SubmitBatchSet(std::vector<SetItem> items) {
  ...
  for (size_t i = 0; i < items.size(); ++i) {
    ...
    // registered LMCache pool we tag the block with its rkey, so
    // EnsureRegistered keeps it as-is and the server RDMA_READs the arena
    // directly; otherwise the cache layer stages it into its registered client
    // pool (single copy).
    ...
    IOBuffer io;
    if (rkey != 0) {
      io.AppendUserDataWithMeta(
          const_cast<void*>(it.data), it.size, [](void*) {}, rkey);
    } else {
...
uint64_t NativeEngine::SubmitBatchGet(std::vector<GetItem> items) {
  ...
  for (size_t i = 0; i < items.size(); ++i) {
    ...
    uint32_t rkey = (rdma_on && rdma_registry_ != nullptr)
                        ...
                        : 0;
    if (rkey != 0) {
      (*response_bufs)[i].AppendUserDataWithMeta(
          it.dst, it.size, [](void*) {}, rkey);
    } else {
