...
void ApplyFlag(const std::string& name, const std::string& value) {
  ...
  if (result.empty()) {
    throw std::invalid_argument("unknown dingofs gflag '" + name + "'");
  }
}

// Translate a dingofs Status into a (ok, err) pair.
//   - ok / IsExist (key already cached or in-flight by another writer) → ok=true
...
uint64_t NativeEngine::SubmitBatchGet(std::vector<GetItem> items) {
  ...
  auto response_bufs =
      std::make_shared<std::vector<IOBuffer>>(items.size());
...
uint64_t NativeEngine::SubmitBatchExists(std::vector<TensorKey> keys) {
  ...
  auto response_bufs =
      std::make_shared<std::vector<IOBuffer>>(keys.size());
