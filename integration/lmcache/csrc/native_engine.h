...
// SetItem / GetItem describe one slot of a batched async I/O.
// The shared BufferHandle pins the underlying Python buffer until the
// I/O completes (then the deleter / capture drops the ref under the GIL).
struct SetItem {
  TensorKey key;
  const void* data;
  size_t size;
  std::shared_ptr<BufferHandle> pin;  // keep py::buffer alive
};
...
struct GetItem {
  ...
  void* dst;
  size_t size;
  std::shared_ptr<BufferHandle> pin;
};
