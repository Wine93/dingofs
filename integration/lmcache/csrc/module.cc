...
// Build a shared_ptr<BufferHandle> that owns a strong ref to the Python
// buffer object, so the underlying memory survives the async I/O. Caller
// must hold the GIL.
std::shared_ptr<BufferHandle> PinBuffer(const py::handle& obj) {
...
uint64_t SubmitBatchSet(NativeEngine& self, const py::sequence& keys,
  ...
  for (size_t i = 0; i < n; ++i) {
    SetItem it;
    it.key = ToTensorKey(keys[i]);
    py::buffer buf = py::reinterpret_borrow<py::buffer>(bufs[i]);
    py::buffer_info info = buf.request(/*writable=*/false);
    it.data = info.ptr;
    it.size = static_cast<size_t>(info.size) * info.itemsize;
    it.pin = PinBuffer(bufs[i]);
...
PYBIND11_MODULE(_dingofs_native, m) {
  ...
  py::class_<NativeEngine>(m, "RemoteCache")
      .def(py::init([](std::string mds_addrs, std::string cache_group,
             ...
             for (auto kv : extra) {
               opts.extra_flags.emplace(py::cast<std::string>(kv.first),
...
Args:
    ...
    extra: optional dict of gflag overrides, e.g. {"cache_rpc_timeout_ms": "5000"}
