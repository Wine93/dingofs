...
Status RDMAPeer::Start() {
  ...
  // client QP. Both go through the DeviceRegistry keyed by FLAGS_rdma_device,
  // so this guarantees QP lkey ↔ buffer lkey match.
  ...
  infiniband::EndPoint ep{
      infiniband::FLAGS_rdma_device,
      static_cast<uint8_t>(infiniband::FLAGS_rdma_port_num),
  };
