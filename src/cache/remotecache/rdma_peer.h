...
Response<U> RDMAPeer::SendRequest(const Request<T>& request) {
  ...
  // For requests with a body (Put / Cache), copy the source bytes into the
  // RDMA-registered buffer so the server can RDMA_READ from a registered
  // region. This is the single client-side copy noted in the design
  // document (§E.2) — FUSE write buffers are not RDMA-registered, so a
  // bounce through the registered pool is unavoidable here.
