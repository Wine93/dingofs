...
DEFINE_string(dingofs_rdma_device, "mlx5_0",
              "IB device used by the DingoFS client-side RDMA buffer pool. "
              "Must match the device used by client QPs (Dialer::Create) so "
              "that lkey and QP share a PD.");
DEFINE_uint32(dingofs_rdma_port_num, 1,
              "HCA port (1-based) used by the DingoFS client-side RDMA buffer "
              "pool");
...
Status ClientRDMAPool::EnsureInitialized() {
  std::call_once(once_, [this]() {
    PoolEndPoint ep{
        FLAGS_dingofs_rdma_device,
        static_cast<uint8_t>(FLAGS_dingofs_rdma_port_num),
    };
    ...
    LOG(INFO) << "ClientRDMAPool initialized: device="
              << FLAGS_dingofs_rdma_device
