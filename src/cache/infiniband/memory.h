...
struct Buffer {
  ...
  uint32_t lkey{0};
  uint32_t rkey{0};
  // io_uring fixed-buffer index for this region after the buffer's owning
...
class RDMAMemoryPool {
 public:
  ...
  // After this pool's iovecs have been registered with io_uring at
  // `index_offset` (the per-iovec id base), assign each Buffer's
  // io_uring_buf_index. Idempotent if called more than once with the same
  // offset.
  void AssignIoUringIndices(int index_offset);
