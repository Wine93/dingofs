...
DECLARE_uint32(rdma_rpc_timeout_ms);
...
class Controller : public ::google::protobuf::RpcController {
 public:
  ...
  // ---- Final transport result (read by the per-RPC blocking call site
  // after the corresponding `Wait*` returns). ----
  const dingofs::Status& WcStatus() const { return wc_status_; }

  // ---- Transport-internal completion notifications ----
  // Each `Notify*` is the producer side of a single one-shot handshake:
  // updates the relevant state then wakes the matching `Wait*`. Called
  // from the CQ-handling path of ClientSession / ServerSession; never
  // by user service code.
  void NotifyRequestSent(const dingofs::Status& wc_status) {
    wc_status_ = wc_status;
    request_sent_.signal();
  }
  void NotifyResponseSent(const dingofs::Status& wc_status) {
    wc_status_ = wc_status;
    response_sent_.signal();
  }
  void NotifyResponseReceived(Buffer* buffer) {
    wc_status_ = dingofs::Status::OK();
    response_buffer_ = buffer;
    response_received_.signal();
  }
  void NotifyResponseError(const dingofs::Status& wc_status) {
    wc_status_ = wc_status;
    response_received_.signal();
  }
  void NotifyRdmaReadDone(const dingofs::Status& wc_status) {
    wc_status_ = wc_status;
    rdma_read_done_.signal();
  }

  Status WaitRequestSent(uint32_t timeout_ms = 0) {
    return Wait(&request_sent_, timeout_ms, "wait request sent timeout");
  }
  Status WaitResponseSent(uint32_t timeout_ms = 0) {
    return Wait(&response_sent_, timeout_ms, "wait response sent timeout");
  }
  Status WaitResponseReceived(uint32_t timeout_ms = 0) {
    return Wait(&response_received_, timeout_ms,
                "wait response received timeout");
  }

 private:
  static Status Wait(bthread::CountdownEvent* event, uint32_t timeout_ms,
