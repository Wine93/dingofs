...
namespace {
void UnregisterRegisteredMemory(uintptr_t addr, size_t length, uint32_t lkey,
                                uint32_t rkey);
}  // namespace

RDMARegion::~RDMARegion() {
  if (mr != nullptr) {
    UnregisterRegisteredMemory(reinterpret_cast<uintptr_t>(base), length, lkey,
                               rkey);
...
std::mutex& RegisteredMemoryMutex() {
  static std::mutex mu;
  return mu;
}

std::vector<RegisteredMemoryRegion>& RegisteredMemoryRegions() {
  static auto* regions = new std::vector<RegisteredMemoryRegion>();
  return *regions;
}

void RegisterRegisteredMemory(const RegisteredMemoryRegion& region) {
  std::lock_guard<std::mutex> lk(RegisteredMemoryMutex());
  RegisteredMemoryRegions().push_back(region);
}

void UnregisterRegisteredMemory(uintptr_t addr, size_t length, uint32_t lkey,
                                uint32_t rkey) {
  std::lock_guard<std::mutex> lk(RegisteredMemoryMutex());
  auto& regions = RegisteredMemoryRegions();
  auto it = std::find_if(regions.begin(), regions.end(),
                         [addr, length, lkey, rkey](
                             const RegisteredMemoryRegion& region) {
                           return region.addr == addr &&
...
Status RegisterRDMAMemoryOnPD(ProtectDomain* pd, void* buffer, size_t length,
  ...
  region->base = buffer;
  region->length = length;
  RegisterRegisteredMemory(
      RegisteredMemoryRegion{reinterpret_cast<uintptr_t>(buffer), length,
                             mr->lkey, mr->rkey});
  *out = std::move(region);
  return Status::OK();
}
...
Status RegisterRDMAMemory(const std::string& device_name, void* buffer,
  ...
  return RegisterRDMAMemory(
      device_name, static_cast<uint8_t>(FLAGS_dingofs_rdma_port_num), buffer,
      length, out);
}
...
bool FindRegisteredRDMAMemory(const void* buffer, size_t length,
  ...
  std::lock_guard<std::mutex> lk(RegisteredMemoryMutex());
  for (const auto& region : RegisteredMemoryRegions()) {
...
void RDMAFixedBufferRegistry::FinalizeIndexAssignment(int read_base_offset,
                                                     int write_base_offset) {
  int read_cursor = read_base_offset;
  int write_cursor = write_base_offset;
  for (size_t i = 0; i < snapshot_.size(); ++i) {
    snapshot_[i]->AssignIoUringIndices(read_cursor, write_cursor);
    read_cursor += static_cast<int>(snapshot_counts_[i]);
    write_cursor += static_cast<int>(snapshot_counts_[i]);
  }
  snapshot_.clear();
  snapshot_counts_.clear();
}
