...
bool VerifyPattern(const char* data, size_t size, uint64_t seed,
  ...
  return pos == size ||
         std::memcmp(data + pos, expected, size - pos) == 0;
}

void SetMemoryContext(Controller* cntl, const Buffer* buffer,
                      uint32_t advertised_length) {
  cntl->SetRequestMemoryContext(
      MemoryContext{reinterpret_cast<uint64_t>(buffer->data),
                    advertised_length, buffer->rkey});
}
...
RoundResult CallPing(Client* client) {
  ...
  auto status = client->Call(&cntl, kServiceName, "Ping", request, &response);

  timer.stop();
  return RoundResult{status.ok(), static_cast<uint64_t>(timer.u_elapsed()), 0,
                     status.ok() ? "" : status.ToString()};
}

RoundResult CallRdmaRead(Client* client, Buffer* buffer, size_t payload_size,
                         uint64_t seed, VerifyMode verify_mode) {
  ...
  Controller cntl;
  SetMemoryContext(&cntl, buffer, static_cast<uint32_t>(payload_size));
  auto status = client->Call(&cntl, kServiceName, "Put", request, &response);
...
RoundResult CallRdmaWrite(Client* client, Buffer* buffer, size_t payload_size,
                          uint64_t seed, VerifyMode verify_mode,
  ...
  Controller cntl;
  SetMemoryContext(&cntl, buffer, advertised_length);
  auto status = client->Call(&cntl, kServiceName, "Range", request, &response);
  ...
  if (!status.ok()) {
    return RoundResult{false, latency_us, 0, status.ToString()};
  }
  if (response.status() != pb_cache::BlockCacheOk) {
    return RoundResult{false, latency_us, 0,
                       "server rejected RDMA_WRITE request"};
  }
  if (cntl.ResponseAttachmentLength() != payload_size) {
    return RoundResult{false, latency_us, 0,
                       absl::StrFormat("attachment length mismatch: got=%u "
                                       "expected=%zu",
                                       cntl.ResponseAttachmentLength(),
                                       payload_size)};
  }
  if (!VerifyPattern(buffer->data, payload_size, seed, verify_mode)) {
    return RoundResult{false, latency_us, 0,
...
RoundResult CallExpectedDispatchFailure(Client* client, Op op) {
  ...
  if (op == Op::kInvalidService) {
    ...
    status =
        client->Call(&cntl, kServiceName, kInvalidMethodName, request,
                     &response);
  }
...
RoundResult RunOne(Client* client, Buffer* buffer, Op op, int thread_id,
...
Summary BuildSummary(const std::vector<WorkerStats>& per_thread,
  ...
  summary.qps = wall_s > 0 ? static_cast<double>(summary.total_rpcs) / wall_s
                           : 0;
  summary.mib_s =
      wall_s > 0
          ? static_cast<double>(summary.bytes) / (1024.0 * 1024.0) / wall_s
          : 0;
...
int main(int argc, char** argv) {
  ...
  RDMAMemoryPoolUPtr mem_pool =
      RDMAMemoryPool::Create(client->GetProtectDomain(), buffer_size,
  ...
  auto worker = [&](int tid) {
    ...
    for (int i = 0; i < FLAGS_warmup_rounds; ++i) {
      auto result =
          RunOne(client.get(), buffer, op, tid, i, payload_size, verify_mode);
      if (!result.ok) {
