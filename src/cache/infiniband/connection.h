...
class Connection {
 public:
  using CompletionHandler = std::function<void(CompletionBatch batch)>;
  ...
  bool HandleCompletion(CompletionHandler handler);

  int GetFd() const { return completion_queue_->GetFd(); }
  QueuePair* GetQueuePair() const { return queue_pair_.get(); }
  RDMAMemoryPool* GetSendMemPool() { return send_mem_pool_.get(); }
  RDMAMemoryPool* GetRecvMemPool() { return recv_mem_pool_.get(); }

 private:
  bool PollCompletionQueue(CompletionHandler handler);

  RDMAMemoryPoolUPtr send_mem_pool_;
  RDMAMemoryPoolUPtr recv_mem_pool_;
