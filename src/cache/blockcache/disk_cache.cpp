...
using LogGuard = cache::common::LogGuard;
...
DiskCache::DiskCache(DiskCacheOptions options)
  ...
  fs_ = std::make_shared<LocalFileSystem>([&](Errno rc) {
    if (rc == Errno::OK) {
...
Errno DiskCache::Cache(const BlockKey& key, const Block& block) {
  ...
  timer.NextPhase(Phase::WRITE_FILE);
  // FIXME
  // rc = fs_->WriteFile(GetCachePath(key), block.data, block.size, true);
  if (rc != Errno::OK) {
    return rc;
  }
...
Errno DiskCache::Load(const BlockKey& key, uint64_t offset, uint64_t length,
                      butil::IOBuf* buffer) {
  CHECK(offset % SysConf::GetPageSize() == 0);
  ...
  auto deleter = [this, fd, offset, length](void* data) {
    auto rc = Posix::MUnmap(data, length);
    if (rc == Errno::OK) {
