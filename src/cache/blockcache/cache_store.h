...
struct Block {
  Block(butil::IOBuf data) : data(data) {}
...
class BlockReader {
 public:
  ...
  virtual Errno ReadAt(off_t offset, size_t length, char* buffer) = 0;
...
class CacheStore {
 ...
 public:
  ...
  virtual Errno Init(UploadFunc uploader) = 0;

  virtual Errno Shutdown() = 0;

  virtual Errno Stage(const BlockKey& key, const Block& block,
                      BlockContext ctx) = 0;

  virtual Errno RemoveStage(const BlockKey& key, BlockContext ctx) = 0;

  virtual Errno Cache(const BlockKey& key, const Block& block) = 0;

  virtual Errno Load(const BlockKey& key,
                     std::shared_ptr<BlockReader>& reader) = 0;

  virtual Errno Load(const BlockKey& key, uint64_t offset, uint64_t length,
