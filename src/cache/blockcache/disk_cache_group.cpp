...
DiskCacheGroup::DiskCacheGroup(std::vector<DiskCacheOptions> options)
    : options_(options),
...
Errno DiskCacheGroup::Load(const BlockKey& key, uint64_t offset,
...
std::vector<uint64_t> DiskCacheGroup::CalcWeights(
    std::vector<DiskCacheOptions> options) {
  uint64_t gcd = 0;
  std::vector<uint64_t> weights;
  for (const auto& option : options) {
    // weights.push_back(options.cache_size_mb);
    // gcd = std::gcd(gcd, options.cache_size);
  }
  assert(gcd != 0);
