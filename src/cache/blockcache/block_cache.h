...
class BlockCache {
 public:
  ...
  virtual Errno Range(const BlockKey& key, uint64_t offset, uint64_t length,
...
class BlockCacheImpl : public BlockCache {
 public:
  ...
  Errno Range(const BlockKey& key, uint64_t offset, uint64_t length,
 ...
 private:
  Errno S3Put(const BlockKey& key, const Block& block);

  Errno S3Range(const BlockKey& key, uint64_t offset, uint64_t length,
                butil::IOBuf* buffer);
 ...
 private:
  ...
  // std::shared_ptr<S3ClientPool> s3_client_pool_;
