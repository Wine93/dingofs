...
class DiskCacheMetric {
 public:
  explicit DiskCacheMetric(DiskCacheOptions options)
      : options_(options),
        metric_(
            StrFormat("dingofs_block_cache.disk_caches_%d", options.index)) {}

  virtual ~DiskCacheMetric() = default;

  void Init() {
    metric_.dir.set_value(options_.cache_dir);
    metric_.used_bytes.set_value(0);
    metric_.capacity.set_value(options_.cache_size_mb * kMiB);
...
struct DiskCacheMetricGuard {
  explicit DiskCacheMetricGuard(Errno* rc, InterfaceMetric* metric,
                                size_t count)
