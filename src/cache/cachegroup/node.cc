...
Status CacheNode::Range(const BlockHandle& handle, off_t offset, size_t length,
...
Status CacheNode::AsyncPrefetch(const BlockHandle& handle, size_t length) {
  if (!IsRunning()) {
    return Status::CacheDown("cache node is down");
  }
  ...
  block_cache_->AsyncPrefetch(
      handle, length, [filename = std::move(filename)](Status status) {
        if (!status.ok()) {
          LOG(ERROR) << "Fail to async prefetch block, key=" << filename
