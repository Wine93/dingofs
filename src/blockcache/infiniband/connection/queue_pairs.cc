...
StatusOr<QueuePairGroup> QueuePairGroup::Create(
  ...
  // msg queue pair
  {
    ...
    option.max_send_wr = MsgSendWr();
    option.max_recv_wr = MsgRecvWr();
...
Status QueuePairGroup::ModifyToReady(std::span<const QueuePairInfo> peers) {
  ...
  Status status = ModifyToReady(&msg_.queue_pair, peers[0]);
  if (!status.ok()) {
    return status;
  }

  for (size_t i = 0; i < bulks_.size(); ++i) {
    status = ModifyToReady(&bulks_[i].queue_pair, peers[i + 1]);
    if (!status.ok()) {
      return status;
    }
  }
...
Status QueuePairGroup::ModifyToReady(QueuePair* queue_pair,
                                     const QueuePairInfo& peer) {
  Status status = queue_pair->ModifyToInit();
  if (!status.ok()) {
    return status;
  }

  status = queue_pair->ModifyToRtr(peer);
  if (!status.ok()) {
    return status;
  }

  status = queue_pair->ModifyToRts();
  if (!status.ok()) {
    return status;
  }
