...
struct QueuePairOption {
  ...
  uint32_t max_send_sge = 2;  // header + payload, gathered in one WR
...
struct __attribute__((packed)) QueuePairInfo {
  uint32_t qpn;
  uint32_t psn;
  uint16_t lid;
  uint8_t port_num;
...
static_assert(sizeof(QueuePairInfo) == 32,
              "QueuePairInfo is a protocol structure");
