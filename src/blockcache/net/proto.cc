...
// The framework's own codes travel on both wires, so they are declared in
// types.h and published in the proto; neither may drift from the other.
static_assert(kReplyOk == pb::cache::v2::REPLY_OK);
static_assert(kReplyBadOpcode == pb::cache::v2::REPLY_BAD_OPCODE);
static_assert(kReplyHandlerError == pb::cache::v2::REPLY_HANDLER_ERROR);
static_assert(kReplyTooLarge == pb::cache::v2::REPLY_TOO_LARGE);
static_assert(kReplyBadRequest == pb::cache::v2::REPLY_BAD_REQUEST);
static_assert(kOpUnspecified == pb::cache::v2::OPCODE_UNSPECIFIED);

Opcode OpcodeOf(const google::protobuf::MethodDescriptor* method) {
  return static_cast<Opcode>(
      method->options().GetExtension(pb::cache::v2::opcode));
}
...
void CheckContract(const google::protobuf::ServiceDescriptor* service) {
  ...
  for (int i = 0; i < count; ++i) {
    ...
    CHECK(method->options().HasExtension(pb::cache::v2::opcode))
